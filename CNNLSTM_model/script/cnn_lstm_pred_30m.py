...
class CNNLSTMModel(nn.Module):
    def __init__(self, input_dim, cnn_out_channels=32, kernel_size=5, hidden_dim=0, fc_dim=0):
    ...
    def forward(self, x):
        ...
        # CNN 層：需要轉換為 [batch_size, input_dim, seq_length]，以符合 Conv1d 格式
        x = x.permute(0, 2, 1)  # 轉換為 [batch_size, input_dim, seq_length]
        x = self.conv1d(x)  # 卷積層
        x = self.relu(x)
        x = x.permute(0, 2, 1)  # 轉回 [batch_size, seq_length, cnn_out_channels]
...
def search_best_LU(sub_train_data, val_data, fixed_DU, fixed_SL,
    ...
    for LU in candidate_LU:
        model = CNNLSTMModel(input_dim=1, hidden_dim=LU, fc_dim=fixed_DU).to(device)
...
def search_best_DU(sub_train_data, val_data, best_LU, fixed_SL,
    ...
    for DU in candidate_DU:
        model = CNNLSTMModel(input_dim=1, hidden_dim=best_LU, fc_dim=DU).to(device)
        criterion = nn.MSELoss()
        optimizer = optim.Adam(model.parameters(), lr=0.001)
...
def search_best_SL(sub_train_data, val_data, best_LU, best_DU,
    ...
    for SL in candidate_SL:
        ...
        model = CNNLSTMModel(input_dim=1, hidden_dim=best_LU, fc_dim=best_DU).to(device)
        criterion = nn.MSELoss()
        optimizer = optim.Adam(model.parameters(), lr=0.001)
